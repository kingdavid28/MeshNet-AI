...
def parse_backend_node(data: dict) -> MeshNode:
    """
    Robustly parses a JSON dictionary representing a node from any
    MeshNet backend or Cloudant schema and converts it into a MeshNode instance.
    """
    nid = str(data.get("node_id") or data.get("id") or "UNKNOWN")

    battery = data.get("battery_level") or data.get("battery_percentage") or data.get("battery") or 100.0
    try:
    ...
    if is_active is None:
        is_active = data.get("bluetooth_status")
    if is_active is None:
    ...
    dev_type = str(data.get("device_type") or data.get("device") or "unknown")

    is_hq = data.get("has_weather_hq_signal")
    if is_hq is None:
    ...
    lat = data.get("lat") or data.get("latitude") or 0.0
    try:
        lat = float(lat)
    except (ValueError, TypeError):
        lat = 0.0

    lon = data.get("lon") or data.get("longitude") or data.get("lng") or 0.0
    try:
        lon = float(lon)
    except (ValueError, TypeError):
        lon = 0.0
