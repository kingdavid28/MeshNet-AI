...
class HardwareManager(EventDispatcher):
    ...
    def enable_hotspot(self) -> None:
        """Request the OS to enable Wi-Fi Hotspot / LocalOnlyHotspot (Android 8.0+)."""
        if platform != "android" or self._wifi_manager is None:
            logger.info("[HW] Non-Android / Stub hotspot enabled programmatically.")
            return
        try:
            from jnius import autoclass, PythonJavaClass, java_method

            class LocalOnlyHotspotCallback(PythonJavaClass):
                __javainterfaces__ = [
                ...
                def __init__(self, manager):
                    super(LocalOnlyHotspotCallback, self).__init__()
                    self.manager = manager

                @java_method('(Landroid/net/wifi/WifiManager$LocalOnlyHotspotReservation;)V')
                def onStarted(self, reservation):
                    logger.info("[HW] LocalOnlyHotspotCallback: Hotspot started successfully!")
                    try:
                        config = reservation.getWifiConfiguration()
                        logger.info("[HW] Hotspot SSID: %s, Passphrase: %s", config.SSID, config.preSharedKey)
                    except Exception as e:
                        logger.error("[HW] Error reading reservation config: %s", e)

                @java_method('()V')
                def onStopped(self):
                    logger.info("[HW] LocalOnlyHotspotCallback: Hotspot stopped.")

                @java_method('(I)V')
                def onFailed(self, reason):
                    logger.error("[HW] LocalOnlyHotspotCallback: Hotspot activation failed. Reason code: %d", reason)

            # Keep a strong reference to callback to prevent garbage collection
            self._hotspot_callback = LocalOnlyHotspotCallback(self)

            # In Android, WifiManager.startLocalOnlyHotspot(callback, handler)
            # Passing None as handler will run it on the main Looper thread.
            self._wifi_manager.startLocalOnlyHotspot(self._hotspot_callback, None)
            logger.info("[HW] WifiManager.startLocalOnlyHotspot requested.")
        except Exception as exc:
            logger.error("[HW] enable_hotspot error: %s", exc)
